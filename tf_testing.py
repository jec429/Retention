...
X_merged = pd.read_pickle("./data_files/old_comb_x_numeric_old_data.pkl")
...
train_labels = np.array(train_df.pop('Vol_Flag'))
val_labels = np.array(val_df.pop('Vol_Flag'))
test_labels = np.array(test_df.pop('Vol_Flag'))
...
#
# # X_merged = pd.read_pickle("./data_files/old_comb_x_numeric_old_data.pkl")
# # new_test_df = X_merged
# #
# # new_test_labels = np.array(new_test_df.pop('Vol_Flag'))
...
#     print(name, ': ', value)
#
# predicted_labels = resampled_model.predict(test_features)
#
# y_true, y_pred = test_labels, [x[0] for x in predicted_labels]
#
...
#
# cm = confusion_matrix(test_labels, np.round(predicted_labels))
# cm = confusion_matrix(test_labels, [[1] if x[0] > 0.2 else [0] for x in predicted_labels])
#
# plt.matshow(cm, alpha=0)
# plt.title('Confusion matrix')
# plt.ylabel('Actual label')
