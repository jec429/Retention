...
def clean_dataframe(year):
    ...
    if year == '2020':
        ...
        df_filtered_filtered.to_csv('data_files/' + year + '_pre_clean.csv', sep=',', encoding='utf-8')
...
def fix_moves_by_year(y1, y2):
    ...
    if year2 == '2020':
        ...
        if path.exists('data_files/' + year1 + '_pre_clean.csv') and path.exists('data_files/' + year2 + '_pre_clean.csv'):
            print('Moving from '+year2+' to '+year1)
            df1 = pd.read_csv('data_files/' + year1 + '_pre_clean.csv', sep=',')
    ...
    for w1 in df1.index:
        ...
        for w2 in df2.index:
            if w1 == w2:
                if df1.at[w1, 'Manager_Manager_WWID'] != df2.at[w2, 'Manager_Manager_WWID']:
                    smc = 1
