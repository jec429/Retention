...
def split_files():
    df_original = pd.read_excel('./data_files/Brazil Retention Dataset_07192019_2015 change field added.xlsx', sheet_name='Data')
...
def clean_dataframe(year):
    ...
    df_2020 = pd.read_excel('./data_files/BRAZIL/Brazil_retention_june2019data_07312019_2018 change field added.xlsx',
                            sheet_name='Data')
    ...
    if year == '2016':
        ...
        df2['Planned_as_a___of_Merit_Tar'] = 0  # df['_015_Planned_Merit_Percent']
        df2['Mgr_Change'] = [1 if x > 0 else 0 for x in df['Mgr_Change_2015']]  # .map(lambda x: x > 0)
        # df2['SkipLevel_Mgr_Change'] = df['SkipLevel_Mgr_Change_2015']  # .map(lambda x: x > 0)
        df2['Promotion'] = [1 if x > 0 else 0 for x in df['Promo_2015']]
        df2['Demotion'] = [1 if x > 0 else 0 for x in df['Demo_2015']]
        df2['Lateral'] = [1 if x > 0 else 0 for x in df['Lateral_2015']]
    elif year == '2017':
        df2['Planned_as_a___of_Bonus_Tar'] = df['_016_Planned_as_a___of_Bonus_Tar']
        df2['Planned_as_a___of_Merit_Tar'] = df['_016_Planned_Merit_Percent']
        df2['Mgr_Change'] = [1 if x > 0 else 0 for x in df['Mgr_Change_2016']]  # .map(lambda x: x > 0)
        # df2['SkipLevel_Mgr_Change'] = df['SkipLevel_Mgr_Change_2016']  # .map(lambda x: x > 0)
        df2['Promotion'] = [1 if x > 0 else 0 for x in df['Promo_2016']]
        df2['Demotion'] = [1 if x > 0 else 0 for x in df['Demo_2016']]
        df2['Lateral'] = [1 if x > 0 else 0 for x in df['Lateral_2016']]
    elif year == '2018':
        df2['Planned_as_a___of_Bonus_Tar'] = df['_017_Planned_as_a___of_Bonus_Tar']
        df2['Planned_as_a___of_Merit_Tar'] = df['_017_Planned_Merit_Percent']
        df2['Mgr_Change'] = [1 if x > 0 else 0 for x in df['Mgr_Change_2017']]  # .map(lambda x: x > 0)
        # df2['SkipLevel_Mgr_Change'] = df['SkipLevel_Mgr_Change_2017']  # .map(lambda x: x > 0)
        df2['Promotion'] = [1 if x > 0 else 0 for x in df['Promo_2017']]
        df2['Demotion'] = [1 if x > 0 else 0 for x in df['Demo_2017']]
        df2['Lateral'] = [1 if x > 0 else 0 for x in df['Lateral_2017']]
    elif year == '2019':
        df2['Planned_as_a___of_Bonus_Tar'] = df['_018_Planned_as_a___of_Bonus_Tar']
        df2['Planned_as_a___of_Merit_Tar'] = df['_018_Planned_Merit_Percent']
        df2['Mgr_Change'] = [1 if x > 0 else 0 for x in df['Mgr_Change_2018']]  # .map(lambda x: x > 0)
        # df2['SkipLevel_Mgr_Change'] = df['SkipLevel_Mgr_Change_2018']  # .map(lambda x: x > 0)
        df2['Promotion'] = [1 if x > 0 else 0 for x in df['Promo_2018']]
        df2['Demotion'] = [1 if x > 0 else 0 for x in df['Demo_2018']]
        df2['Lateral'] = [1 if x > 0 else 0 for x in df['Lateral_2018']]
    elif year == '2020':
    ...
    clean_loc = []
    for x in df_filtered_filtered['Location_Code__IA__Host_All_Othe']:
        floc = int(x.split(' ')[0][2:])
        floc = 'BR' + str(floc)
        clean_loc.append(floc)

    df_filtered_filtered['Location_Code__IA__Host_All_Othe'] = clean_loc
    df_filtered_filtered.to_csv('data_files/BRAZIL/' + year + '_fixed.csv', sep=',', encoding='utf-8')


def pickle_dataframe():
    from preprocessing import OneHotEncoder

    df_name = 'merged_Brazil_combined_fixed'
    ...
    data_x_numeric.to_pickle("./data_files/BRAZIL/"+df_name+"_x_numeric_newer2.pkl")
    data_x_numeric.to_csv("./data_files/BRAZIL/" + df_name + "_x_numeric_newer2.csv", sep=',', encoding='utf-8')
...
def fix_moves_by_year(y1, y2):
    ...
    if year2 == '2020':
        if path.exists('data_files/' + year1 + '_pre_clean.csv') and path.exists(
            ...
            df2 = pd.read_csv('data_files/' + year2 + '_clean.csv', sep=',')
        else:
            print('Moving from ' + year2 + ' to ' + year1)
            print('Files not available')
            return
    else:
        if path.exists('data_files/BRAZIL/' + year1 + '_pre_fixed.csv') and \
                path.exists('data_files/BRAZIL/' + year2 + '_pre_fixed.csv'):
            print('Moving from '+year2+' to '+year1)
            df1 = pd.read_csv('data_files/BRAZIL/' + year1 + '_pre_fixed.csv', sep=',')
            df2 = pd.read_csv('data_files/BRAZIL/' + year2 + '_pre_fixed.csv', sep=',')
        else:
            print('Files not available')
            return
    ...
    for w1 in df1.index:
        ...
        for w2 in df2.index:
            if w1 == w2:
                ...
                if df1.at[w1, 'Manager_WWID__IA__Host_All_Other'] != df2.at[w2, 'Manager_WWID__IA__Host_All_Other'].any():
                    smc = 1
                if df1.at[w1, 'Employee_Pay_Grade'] == df2.at[w2, 'Employee_Pay_Grade']:
                    jbidc = df1.at[w1, 'Job_ID'] != df2.at[w2, 'Job_ID']
                    pstnc = (df1.at[w1, 'Position'] != df2.at[w2, 'Position']) and \
                            (df1.at[w1, 'Position_ID'] != df2.at[w2, 'Position_ID'])
                    if jbidc or pstnc:
                        # print(w1, jbidc, pstnc)
                        lat = 1
        manager_change.append(smc)
        lateral.append(lat)

    df1['Mgr_Change'] = manager_change
    df1 = df1.drop(['Job_ID', 'Position', 'Position_ID'], axis=1)
    if int(year1) > 2012 and int(year2) > 2012:
        df1['Lateral'] = lateral

    df1.to_csv('data_files/BRAZIL/' + year1 + '_fixed.csv', sep=',', encoding='utf-8')
...
def merge_files():
    ...
    df_2011 = pd.read_csv('data_files/BRAZIL/2011_fixed.csv', sep=',')
    df_2012 = pd.read_csv('data_files/BRAZIL/2012_fixed.csv', sep=',')
    df_2013 = pd.read_csv('data_files/BRAZIL/2013_fixed.csv', sep=',')
    df_2014 = pd.read_csv('data_files/BRAZIL/2014_fixed.csv', sep=',')
    df_2015 = pd.read_csv('data_files/BRAZIL/2015_fixed.csv', sep=',')
    df_2016 = pd.read_csv('data_files/BRAZIL/2016_fixed.csv', sep=',')
    df_2017 = pd.read_csv('data_files/BRAZIL/2017_fixed.csv', sep=',')
    df_2018 = pd.read_csv('data_files/BRAZIL/2018_fixed.csv', sep=',')
    df_2019 = pd.read_csv('data_files/BRAZIL/2019_fixed.csv', sep=',')
    ...
    df_merged.to_csv('data_files/BRAZIL/merged_Brazil_combined_fixed.csv', sep=',', encoding='utf-8')
...
def pickle_current_file():
    ...
    df_original = df_original[df_original['Employee_Type__IA__Host_All_Othe'].str.contains('Fixed Term') == False]
    df_original = df_original[df_original['Employee_Type__IA__Host_All_Othe'].str.contains('Intern') == False]
    df_original.info()
    df = pd.DataFrame()
    ...
    df['Planned_as_a___of_Merit_Tar'] = df_original['_018_Planned_Merit_Percent']
    ...
    df['Total_Base_Pay'] = df_original['Total_Base_Pay___US']
    ...
    clean_loc = []
    for x in df['Location_Code__IA__Host_All_Othe']:
        floc = int(x.split(' ')[0][2:])
        floc = 'BR' + str(floc)
        clean_loc.append(floc)

    df['Location_Code__IA__Host_All_Othe'] = clean_loc
    ...
    data_x_numeric.info()
    data_x_numeric = data_x_numeric.fillna(-1)
    ...
    data_x_numeric.to_pickle("./data_files/BRAZIL/" + df_name + "_current_x_numeric_newer2.pkl")
    data_x_numeric.to_csv("./data_files/BRAZIL/" + df_name + "_current_x_numeric_newer2.csv", sep=',', encoding='utf-8')


if __name__ == '__main__':
    # split_files()
    # write_to_pickle('2009')
    # write_to_pickle('2010')
    # write_to_pickle('2011')
    # write_to_pickle('2012')
    # write_to_pickle('2013')
    # write_to_pickle('2014')
    # write_to_pickle('2015')

    # clean_dataframe_old('2009')
    # clean_dataframe_old('2010')
    # clean_dataframe_old('2011')
    # clean_dataframe_old('2012')
    ...
    #
    # fix_moves_by_year('2010', '2011')
    # fix_moves_by_year('2011', '2012')
    # fix_moves_by_year('2012', '2013')
    # fix_moves_by_year('2013', '2014')
    # fix_moves_by_year('2014', '2015')
